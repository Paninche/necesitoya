  ...
  return (
    <main style={{fontFamily: "Arial, sans-serif", margin: 0, padding: 0}}>
      ...
      <section style={{background: "linear-gradient(135deg, #1a1a2e 0%, #0f3460 100%)", padding: "80px 32px", textAlign: "center"}}>
        <div style={{fontSize: "14px", color: "#FF6B35", letterSpacing: "4px", marginBottom: "16px"}}>FLORIDA'S BILINGUAL SERVICES APP</div>
      ...
      <section style={{padding: "60px 32px", background: "#f8f6f2"}}>
        <h2 style={{textAlign: "center", fontSize: "28px", color: "#1a1a2e", marginBottom: "8px"}}>What do you need? · ¿Qué necesitas?</h2>
        ...
        <div style={{display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "16px", maxWidth: "800px", margin: "0 auto"}}>
          {[
            {icon: "🚛", label: "Hauling & Pickup", es: "Mudanza"},
            {icon: "🔨", label: "Handyman", es: "Reparaciones"},
            {icon: "🌿", label: "Lawn & Garden", es: "Jardín"},
            {icon: "🧹", label: "Cleaning", es: "Limpieza"},
            {icon: "📦", label: "Moving Boxes", es: "Cajas"},
      ...
      <section style={{padding: "60px 32px", background: "white"}}>
        <h2 style={{textAlign: "center", fontSize: "28px", color: "#1a1a2e", marginBottom: "40px"}}>How it works · Cómo funciona</h2>
        <div style={{display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "32px", maxWidth: "800px", margin: "0 auto"}}>
          {[
            {step: "1", icon: "📋", title: "Post your need", sub: "Describe what you need, set your budget"},
            {step: "2", icon: "🔔", title: "Get offers", sub: "Nearby providers respond fast"},
            {step: "3", icon: "✅", title: "Pick & pay", sub: "Choose the best offer, pay securely in-app"},
          ].map((s) => (
            <div key={s.step} style={{textAlign: "center"}}>
              ...
              <div style={{fontWeight: "bold", color: "#1a1a2e", marginBottom: "4px"}}>{s.title}</div>
              <div style={{color: "#888", fontSize: "14px"}}>{s.sub}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{background: "linear-gradient(135deg, #1a1a2e, #0f3460)", padding: "60px 32px", textAlign: "center"}}>
        <h2 style={{color: "white", fontSize: "32px", marginBottom: "8px"}}>Be the first to know</h2>
