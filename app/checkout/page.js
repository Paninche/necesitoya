...
export default function Checkout() {
  ...
  const commission = Math.round(parseFloat(amount || 0) * 0.08);
  const providerAmount = parseFloat(amount || 0) - commission;
  ...
  async function handlePayment(e) {
    ...
    try {
      ...
      // Payment intent created — redirect to success
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f9fafb', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '40px', maxWidth: '480px', width: '100%', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          <div style={{ fontSize: '28px', fontWeight: '800', color: '#1a1a2e', marginBottom: '4px' }}>
            NecesitoYa
          </div>
        ...
        <form onSubmit={handlePayment}>
          ...
          {error && (
            <div style={{ backgroundColor: '#fee2e2', color: '#dc2626', padding: '12px', borderRadius: '8px', fontSize: '14px', marginBottom: '16px' }}>
              {error}
            </div>
          )}
          ...
          <button
            type="submit"
            disabled={loading}
            style={{ width: '100%', backgroundColor: loading ? '#9ca3af' : '#2563eb', color: 'white', padding: '14px', borderRadius: '8px', fontSize: '16px', fontWeight: '600', border: 'none', cursor: loading ? 'not-allowed' : 'pointer' }}
          >
